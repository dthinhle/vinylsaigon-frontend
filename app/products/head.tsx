...
export default async function Head({ searchParams = {} }: Props) {
  const page =
  ...
  const baseUrl = `${process.env.NEXT_PUBLIC_APP_URL?.replace(/\/$/, '') || ''}/san-pham`
  ...
  // Determine if next page exists by querying backend products API
  try {
    const nextPage = page + 1
    const apiParams = new URLSearchParams()
    // copy preserved params to api request
