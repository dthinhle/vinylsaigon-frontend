...
export async function POST(request: NextRequest) {
  try {
    ...
    if (type === 'all') {
      ...
      return NextResponse.json({
        message: 'All product pages revalidated successfully',
    ...
    if (!slug) {
      return NextResponse.json(
        { error: 'Slug is required for specific product revalidation' },
        { status: 400 },
      )
    }
    ...
    console.log(`Revalidating product with slug: ${sanitizedSlug}`)
    revalidatePath(`/products/${sanitizedSlug}`)
    revalidateTag(`product-${sanitizedSlug}`)

    return NextResponse.json({
      message: 'Product revalidated successfully',
      revalidatedAt: new Date().toISOString(),
      slug: sanitizedSlug,
      type: 'specific',
    })
    ...
    return NextResponse.json(
      {
        error: 'Failed to revalidate product cache',
...
export async function GET() {
  return NextResponse.json({
    message: 'Product revalidation endpoint is working. Use POST to trigger revalidation.',
    endpoint: '/api/revalidate-product',
    method: 'POST',
    headers: {
      Authorization: 'Bearer YOUR_REVALIDATION_SECRET (optional)',
    },
    body: {
      slug: 'product-slug (required if type is "specific", optional if type is "all")',
