...
import { ICollection, IRelatedCategory } from '@/lib/types/global'
...
interface ListProductByCollectionProps {
  collection: ICollection
  parentCategory?: IRelatedCategory
  activeCategory: IRelatedCategory | ICategoryBase | undefined
}

export const ListProductByCollection = ({ collection, activeCategory, parentCategory }: ListProductByCollectionProps) => {
  const filters = [`collections = "${collection.name}"`]
  if (activeCategory) {
  ...
  return (
    ...
    >
      ...
      <div className='filter-wrapper'>
        <FilterBar type='collection' />
        <FilterPanel type='collection' />
      </div>
